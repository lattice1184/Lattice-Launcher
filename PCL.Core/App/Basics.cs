...
public static class Basics
{
    ...
    {
        // [Avalonia 补丁] 原实现依赖 WPF Application 容器，改为读取嵌入资源
        var assembly = Assembly.GetEntryAssembly() ?? typeof(Basics).Assembly;
        return assembly.GetManifestResourceStream(path)
            ?? assembly.GetManifestResourceStream($"{assembly.GetName().Name}.{path}");
    }
