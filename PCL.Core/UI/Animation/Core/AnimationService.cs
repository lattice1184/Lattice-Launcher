...
public sealed class AnimationService : GeneralService
{
    ...
    /// <summary>
    /// [Avalonia 补丁] UI 线程访问提供器工厂，由宿主注入（null 时使用直执行默认实现）
    /// </summary>
    public static Func<IUIAccessProvider>? UIAccessProviderFactory { get; set; }

    /// <summary>
    /// [Avalonia 补丁] 直执行 UI 访问提供器：所有调用在当前线程立即执行，FrameTick 不触发
    /// </summary>
    private sealed class DefaultUIAccessProvider : IUIAccessProvider
    {
    ...
    private static void _Initialize()
    {
        ...
        // [Avalonia 补丁] 初始化 UI 线程访问提供器并启动赋值 Task（可注入）
        UIAccessProvider = UIAccessProviderFactory?.Invoke() ?? new DefaultUIAccessProvider();
