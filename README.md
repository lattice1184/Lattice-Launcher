...
- **常驻占用低**：启动器自身稳态约 190MB 工作集（独占 130MB 上下，托管堆仅 15MB 左右）——正常使用不需要任何手动清理
- **一键释放**：主页或设置 → 存储/模块 有「立即释放内存」按钮——切走时点一下，把驻留物理页还给系统（实测 470MB → 30MB 级），回来用到什么页自动换回，不丢数据
- **自动模式**（可选）：闲置或切走时后台压缩内存——默认只压缩不写盘，对硬盘零影响；「工作集修剪」（PCL 式，写页面文件）默认关，固态盘寿命敏感就保持关闭
- 闲置触发时长可调（1-60 分钟），设置页实时显示当前占用
