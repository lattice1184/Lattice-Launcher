...
/// <summary>
/// 指数退避重试（Polly 风格，无第三方依赖）：0.5s×2ⁿ 上限 30s（国内直连官方源常失败，
/// 快速重试换镜像，避免用户在"卡住感"里等 1s 起步的退避）。
...
public static class RetryPolicy
{
    ...
    public static TimeSpan Backoff(int attemptIndex)
        => TimeSpan.FromSeconds(attemptIndex == 0 ? 0.5 : Math.Min(1L << (attemptIndex - 1), (long)MaxBackoff.TotalSeconds));
