...
{
    private static readonly IReadOnlyDictionary<TerracottaLobbyFailure, (string Reason, string Suggestion, FixKind Fix)> TerracottaMap =
        new Dictionary<TerracottaLobbyFailure, (string, string, FixKind)>
        {
            [TerracottaLobbyFailure.InvalidRoomCode] = (
                "房间码无效（服务器返回 400）。",
            ...
            [TerracottaLobbyFailure.ProtocolFailed] = (
                "联机模块接口异常。",
            ...
            [TerracottaLobbyFailure.RoomConnectionFailed] = (
                ...
                FixKind.AdviceOnly),
            [TerracottaLobbyFailure.Cancelled] = (
                "操作已取消。",
                "",
                FixKind.AdviceOnly),
        };
    ...
    public static IEnumerable<TerracottaLobbyFailure> TerracottaKeys => TerracottaMap.Keys;

    /// <summary>联机失败 → 诊断（Snippet=枚举名；detail 为底层异常消息，嵌入原因）</summary>
    public static DiagnosticHit ForTerracotta(TerracottaLobbyFailure failure, string? detail = null)
    {
