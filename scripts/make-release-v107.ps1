# v1.0.7 release 创建：建 release + 上传两个 exe 资产
...
$exists = $null
try { $exists = Invoke-RestMethod -Method Get -Uri "https://api.github.com/repos/$repo/releases/tags/v1.0.7" -Headers $headers } catch { $exists = $null }
if ($exists) { Write-Host "release v1.0.7 already exists (id=$($exists.id))" } else {
  $body = [IO.File]::ReadAllText((Join-Path $root "body-v107.md"), [Text.Encoding]::UTF8)
  $json = @{ tag_name = "v1.0.7"; name = "Starview Launcher v1.0.7"; body = $body; draft = $false; prerelease = $false } | ConvertTo-Json -Depth 3
...
# 2) 上传资产（二进制）
$rel = Invoke-RestMethod -Method Get -Uri "https://api.github.com/repos/$repo/releases/tags/v1.0.7" -Headers $headers
