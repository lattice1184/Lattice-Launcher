# v1.0.8 release 创建：建 release + 上传两个 exe 资产
...
$exists = $null
try { $exists = Invoke-RestMethod -Method Get -Uri "https://api.github.com/repos/$repo/releases/tags/v1.0.8" -Headers $headers } catch { $exists = $null }
if ($exists) { Write-Host "release v1.0.8 already exists (id=$($exists.id))" } else {
  $body = [IO.File]::ReadAllText((Join-Path $root "body-v108.md"), [Text.Encoding]::UTF8)
  $json = @{ tag_name = "v1.0.8"; name = "Starview Launcher v1.0.8"; body = $body; draft = $false; prerelease = $false } | ConvertTo-Json -Depth 3
...
# 2) 上传资产（二进制）
$rel = Invoke-RestMethod -Method Get -Uri "https://api.github.com/repos/$repo/releases/tags/v1.0.8" -Headers $headers
